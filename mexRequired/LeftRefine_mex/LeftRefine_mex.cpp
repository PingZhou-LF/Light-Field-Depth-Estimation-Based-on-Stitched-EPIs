...
#define maskLeft_m  prhs[4]  //512*512
#define kRes_m prhs[5]       //int
#define sai_m  prhs[6]       //9*9*512*512

#define disRefine_m plhs[0]    //512*512
//#define args plhs[1] 
//#define disRefine_single_m plhs[2]
//#define costDelta_m plhs[3]
//#define cost_m plhs[4]
...
//double test(const mxArray* prhs[], mxArray* plhs[]) {
//    arguments info;
...
//    double Rs = (Ns + 1) / 2;
//    MyMatrix2d maskLeft((int)mxGetM(maskLeft_m), (int)mxGetN(maskLeft_m), mxGetPr(maskLeft_m));
//    vector<int> leftY, leftX;
...
//        if (xTemp[0] >= 0 && notAllZero && noNan) {
//            double costTemp1 = indexCalUp(kTemp[0], xTemp[0], y, info, prhs);
//            double costTemp2 = indexCalUp(kTemp[1], xTemp[1], y, info, prhs);
//            double costTemp3 = indexCalUp(kTemp[2], xTemp[2], y, info, prhs);
//            double costTemp4 = indexCalUp(kTemp[3], xTemp[3], y, info, prhs);
//            double costTemp5 = indexCalUp(kTemp[4], xTemp[4], y, info, prhs);
//            /*double costTemp1 = 1;
//            double costTemp2 = 1;
//            double costTemp3 = 1;
//            double costTemp4 = 1;
//            double costTemp5 = 1;*/
//            sumCostTemp1 = costTemp1 + costTemp2 + costTemp3 + costTemp4 + costTemp5;
...
//    costInd = 0;
//    double kstep = (kmax - kmin) / (kRes * 1.0 - 1);
//    for (int indK = 0; indK <= kRes; indK++) {
//        double k = kmin + indK * kstep;
//        vector<double> kTemp(5, 0);
//        vector<double> xTemp(5, 0);
//        for (int i = 0; i < 5; i++) {
//            kTemp[i] = k / (1 + (i - 4) * delta * k);
//            xTemp[i] = x + i - 4;
//        }
//        bool notAllZero = true, noNan = true;
//        int count0 = 0;
...
//            double costTemp4 = 1;
//            double costTemp5 = 1;*/
//            sumCostTemp2 = costTemp1 * exp(-1) + costTemp2 * exp(-9.0/16.0) + costTemp3 * exp(-4.0 / 16.0) + costTemp4 * exp(-1.0 / 16.0) + costTemp5;
//            cost[costInd] = sumCostTemp2;
//            costInd++;
//        }
//        else {
//            cost[costInd] = 10000;
...
//    args = mxCreateDoubleMatrix(8, 1, mxREAL);
//    double* args_pr = mxGetPr(args);
//    args_pr[0] = kmin; args_pr[1] = kmax; args_pr[2] = deltaInd;
//    args_pr[3] = delta; args_pr[4] = costInd; args_pr[5] = k1;
//    args_pr[6] = sumCostTemp1; args_pr[7] = sumCostTemp2;
//    cost_m = mxCreateDoubleMatrix(kRes + 1, 1, mxREAL);
//    double* cost_pr = mxGetPr(cost_m);
//    costDelta_m = mxCreateDoubleMatrix(201, 1, mxREAL);
//    double* costDelta_pr = mxGetPr(costDelta_m);
//    for (int i = 0; i < kRes + 1; i++) cost_pr[i] = cost[i];
//    for (int i = 0; i < 201; i++) costDelta_pr[i] = costDelta[i];
//    return threshold(1 / k1, info.dis_max, info.dis_min);
//}
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) {
    ...
    /*disRefine_m = mxCreateDoubleMatrix((int)mxGetM(dis_m), (int)mxGetN(dis_m), mxREAL);
    disRefine_single_m = mxCreateDoubleMatrix(1, 1, mxREAL);
    double* disRefine_single_pr = mxGetPr(disRefine_single_m);
    disRefine_single_pr[0] = test(prhs, plhs);*/
}

...
%
clear
% nameAll={'horses','medieval','stilllife'};
% clear
% name='horses';
addpath(genpath(pwd));
name = 'rosemary';
...
% disLinesRefine(isnan(disLinesRefine))=dis_occ(isnan(disLinesRefine));
% figure;imshow(dis0,[],'InitialMagnification','fit');
% figure;imshow(dis_occ,[],'InitialMagnification','fit');

% disLinesRefine1=biFilter(disLinesRefine, info.csai,1,10);
% figure;imshow(disLinesRefine1,[],'InitialMagnification','fit');
dis1=disLinesRefine;
figure(1);imshow(dis1,[],'InitialMagnification','fit');
alpha=0.5;
...
pathResult=['E:\0_research\1_code\lxy\',nameDataset,'\dataMLR\',name,'DisData'];
save(pathResult, 'dis0','dis_occ','dis1','c0','c_occ');

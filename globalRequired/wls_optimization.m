function out = wls_optimization(in, data_weight, guidance, lambda, varargin)
...
% Fattal, Lischinski and Szeliski, "Edge-Pres高erving Decompositions for 
...
% dxt= dxt.*((dxt>Th_textureSupress)+ (dxt<=Th_textureSupress).*2./(1+exp(-5*(dxt-Th_textureSupress))));
%  dxt(find(cwn==1)) = dxt(find(cwn==1)).*2./(1+exp(-10*(dxt(find(cwn==1))-Th_textureSupress)));
end
dx = -lambda./(dxt.^2 + small_num);
dx(find(cwn==1)) = dx(find(cwn==1))*0.1; %低纹理区域的纹理抑制
...
for ne= 1:numElem
    ...
    currIn= in{ne};
    notNanValues=currIn(~isnan(currIn));
    currIn(isnan(currIn))=mean(notNanValues);
